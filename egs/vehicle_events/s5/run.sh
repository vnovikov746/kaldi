...
data=/home/gzlncf/projects/vtools/audio-to-text-training-data/general_driving_conditions
...
if [ $stage -eq 2 ]; then
  ...
  # Get the shortest 20 utterances first because those are more likely
  # to have accurate alignments.
  utils/subset_data_dir.sh --shortest data/train_clean_5 20 data/train_20short
fi
...
if [ $stage -eq 3 ]; then
  ...
  steps/train_mono.sh --boost-silence 1.25 --nj 2 --cmd "$train_cmd" \
    data/train_20short data/lang_nosp exp/mono
...
if [ $stage -eq 5 ]; then
  ...
  steps/align_si.sh  --nj 2 --cmd "$train_cmd" --use-graphs true \
